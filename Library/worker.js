...
const twilioSms = require('../Helpers/Notification');
	...
	data.list('checks', (err1, checks) => {
		if (!err1 && checks && checks.length > 0) {
			checks.forEach((check) => {
				// read the checkData
				data.readfile('checks', check, (err2, originalCheckData) => {
					if (!err2 && originalCheckData) {
						// pass the data to the check validator
						worker.validateCheckData(
							handlerUtilities.parseJSON(originalCheckData)
						);
					} else {
...
worker.alertUserToStatusChange = (newCheckData) => {
	...
	twilioSms.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
		if (!err) {
